# Hot Matcha Package Act (hmpact)

**hmpact** は、既存の JavaScript パッケージマネージャーとは異なるアプローチで  
依存管理とパッケージ配布を再構築することを目指す、新しいパッケージ管理基盤です。

> “Act” は「実行・作用」を意味し、  
> “hmpact” は “ihmpact（影響）” を連想させる語感を持ちます。  
...
hmpact が目指すのは、従来の `package.json` ベースの依存管理の制約を超え、  
...
hmpact はこの隙間を埋めるミドルレイヤーとして設計されます。
...
hmpactはモノレポ構成を採用しており、複数のパッケージで構成されています：

- `hmpact` - コアCLI
- `@hmpact/logger` - ロギングユーティリティ（予定）
- `@hmpact/translationpack` - 多言語対応（予定）

### バージョン管理

**Next.jsと同様の統一バージョン管理**を採用しています。すべてのパッケージは同じバージョン番号を持ち、一緒にリリースされます。
